...
if ($userID !== false) {
    echo "\033[1;33mValid credentials. Welcome.\n";
...
// Counter to track consecutive failures
            ...
            //			
                   ...
                   try {
                                if ($mail->send()) {
                                    ...
                                    $errorMessage = strtolower($mail->ErrorInfo);
                                    $isNotOurCustomer = (strpos($errorMessage, 'not our customer') !== false);
                            
                                    if (!$isNotOurCustomer) {
                                        // Log the email to the failed emails file
                                    ...
                                    if ($consecutiveFailures >= $maxConsecutiveFailures) {
                                        ...
                                        // Update failed list with unsent emails
                                        $sentEmails = file($sentEmailsFile, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
                                        $unsentFiltered = array_diff($recipientList, $sentEmails);
                                        $existingFailedEmails = file($failedEmailsFile, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
                                        $unsentFilteredUnique = array_values(array_diff($unsentFiltered, $existingFailedEmails));
                                        file_put_contents($failedEmailsFile, implode(PHP_EOL, $unsentFilteredUnique) . PHP_EOL, FILE_APPEND);
            ...
            $recipientListFile = 'list/' . $settings['recipientListFile'];  // Update the path as needed
                ...
                function countUniqueLines($file) {
                    return count(array_unique(file($file, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES)));
                }
                
                $failedEmailsCount = countUniqueLines($failedEmailsFile);
                $sentEmailsCount = countUniqueLines($sentEmailsFile);
                
                // Calculate the total count of processed unique emails
                $totalProcessedEmails = $failedEmailsCount + $sentEmailsCount;
                
                // Count the number of unique emails in the original list.txt file
                $originalEmailsCount = countUniqueLines($recipientListFile);
                
                // Check if the total count of processed unique emails matches the count of emails in the original list
                if ($totalProcessedEmails === $originalEmailsCount) {
                    ...
                    $unsentCount = $originalEmailsCount - $totalProcessedEmails;
                    echo "\n\033[0;33mNot all emails have been sent due to Not our customer errors. Unsent Count: $unsentCount\033[0m\n";
                }
